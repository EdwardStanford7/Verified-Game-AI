...
= Abstract
Game AI often feels unfair because perception and planning are implemented with different assumptions. An enemy may react to hidden information or attempt a route that later turns out to be impossible. This project studies a verified version of that problem in a two-dimensional grid world. The goal is simple: an agent should only pursue targets it can currently see, and any returned path should respect the movement constraints of the map. The implementation is written in Dafny and combines rectangle-based field-of-view with A\* search. The current result is a working verified prototype for perception-constrained planning, together with a clear description of which guarantees are already proved and which remain future work.

= Introduction
Logically coherent game AI should operate under the same informational limits as the player. However, this often (to entertaining degrees) fails in practice. A common pattern is to compute a target using complete world state and only afterward ask a planner to justify the choice. Even when the resulting behavior is functional, it can look dishonest: the agent appears to know about hidden goals, to see through walls, or to commit to routes that later need to be repaired.

The goal of this project is to ask whether one narrow but important slice of game AI can be made honest by construction. The setting is a grid world with blocking and non-blocking obstacles, a single agent, and a set of candidate goals. The intended behavior is to compute the cells visible to the agent, select the most valuable visible target, and then produce a path that respects traversability. Rather than treating these as loosely coupled heuristics, our project encodes them in Dafny, a language and verifier for functional correctness (Leino 2010). The visibility component is based on the rectangle field-of-view formulation of Debenham and Solis-Oba (2020), while the planning component uses A\* search (Hart, Nilsson, and Raphael 1968).

The ideal outcome of the project would be achieving three contributions. First, it would provide a reusable verified specification of visibility-aware pathfinding for grid-based agents. Second, it would prove strong end-to-end guarantees connecting visible goal selection to feasible planning. Third, it would show that such a verified core can still support practical integration in systems such as Unity or exported C-sharp code. The current state of our project does not yet achieve the full vision, but it does establish a credible core: visibility, goal choice, and pathfinding are composed in executable Dafny code rather than being left at the level of informal claims.

= Status
The project currently delivers a verified prototype, not a finished library. The strongest completed pieces are rectangle extraction, rectangle-based field-of-view, and an A\* implementation that reconstructs a feasible path through traversable cells. These modules run end-to-end in the provided demo and verify successfully under Dafny.

The main missing piece is the strongest end-to-end contract. The public `FindPath` interface does not yet prove that the chosen destination is the maximum-value visible goal, nor that the path is optimal in the full A\* sense. The implementation also does not yet include performance work, experiments, or engine integration. The rest of this report should therefore be read as documenting a correct and functioning core rather than a finished artifact.

= Demo
The demo ran from `src/main.dfy` builds a `25 x 25` world with four cell types: `Empty`, `High`, `Low`, and `Goal`. `High` cells block both movement and visibility. `Low` cells block movement but not visibility. This separation is important because it lets the example exercise both pathfinding and line-of-sight behavior.
...
= Implementation
The current implementation is organized into four Dafny modules. `src/utils.dfy` defines shared concepts such as points, rectangles, and feasible paths. `src/visibility.dfy` contains the geometric core. It groups blocking cells into rectangles and computes field of view using a doubled-coordinate representation so that rays from cell centers can be reasoned about with integer arithmetic.
...
`src/a_star.dfy` implements 4-connected A\* search. The proof effort there focuses on feasibility rather than shortest-path optimality: if the method returns a non-empty path, then the path starts at the source, ends at the goal, moves only between adjacent cells, and stays within traversable terrain. Finally, `src/pathfind.dfy` exposes the user-facing `Pathfinder<T>` abstraction. Its constructor caches blocking rectangles, and `FindPath` performs the overall pipeline: compute visibility, select a visible goal using a value function, and run A\* to that goal.
